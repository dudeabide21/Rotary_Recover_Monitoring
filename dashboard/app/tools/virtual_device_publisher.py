...
# ========= EDIT THESE =========
PORT = "/dev/tty.usbserial-10"   # change to your real port
BAUD = 115200

INGEST_URL = "https://rot-dash.vercel.app/api/ingest"
DEVICE_TOKEN = "super_secret"
...
class RepFSM:
    ...
    def __init__(self):
        ...
        self.last_dw=None
        self.rep_start=None
        self.above_start=0.0
        self.below_stop=0.0
        self.cooldown=-1e9
        self.rom_last=0.0
        self.rep_dur_last=0.0
        ...
        if self.state==self.IDLE:
            ...
            if self.below_stop >= MIN_REST_S:
                ...
                if dur >= MIN_REP_DUR_S:
                    self.rom_last = self.rom_rad * (180.0 / math.pi)
...
def post_bucket(bucket_start, metrics):
    # Force primitives (avoid numpy weirdness)
    safe = {}
...
def main():
    ...
    while True:
        ...
        if not line or line.startswith("SEQ") or "A1X" in line:
            continue
        ...
        if b != bucket_start and n > 0:
            ...
            code, txt = post_bucket(bucket_start, metrics)
            print(f"POST {bucket_start} HTTP {code} rep {fsm.rep_total} rom {fsm.rom_last:.2f} -> {txt}")
