...
export async function POST(req) {
  ...
  const { deviceId, bucketStart, bucketSec = 5, metrics } = body;

  if (!deviceId || typeof bucketStart !== "number" || !metrics) {
  ...
  await sql`
    insert into telemetry_points (device_id, bucket_start, bucket_sec, metrics)
    values (${deviceId}, ${bucketIso}::timestamptz, ${bucketSec}, ${JSON.stringify(metrics)}::jsonb)
